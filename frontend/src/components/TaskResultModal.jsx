...
function downloadProject(result) {
  ...
  const header = `# ${result.project_name || "Project"}\n# ${result.summary || ""}\n# Generated by Workforce.AI\n\n`;
  const footer = result.setup_instructions ? `\n${'='.repeat(60)}\n🚀 Setup Instructions\n${'='.repeat(60)}\n${result.setup_instructions}\n` : "";
  const blob = new Blob([header + content + footer], { type: "text/plain" });
  const url = URL.createObjectURL(blob);
  const a = document.createElement("a");
  a.href = url;
  a.download = `${result.project_name || "project"}-source.txt`;
  a.click();
